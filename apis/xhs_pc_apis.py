...
class XHS_Apis():
    ...
    def get_user_all_notes(self, user_url: str, cookies_str: str, require_num: int = None, proxies: dict = None):
        """
           获取用户所有笔记
           :param user_id: 你想要获取的用户的id
           :param cookies_str: 你的cookies
           :param require_num: 限制抓取数量 (可选)
           返回用户的所有笔记
        """
        cursor = ''
        note_list = []
        try:
            ...
            while True:
                ...
                # Check limit
                if require_num and len(note_list) >= require_num:
                    note_list = note_list[:require_num]
                    break
