...
1. 运行以下命令启动 Web 界面：
   ...
   python app.py
   ```
2. 访问 http://127.0.0.1:5007
3. 点击 "一键自动获取 (推荐)" 按钮
4. 脚本会自动打开一个浏览器窗口，访问小红书。
5. 请在弹出的窗口中进行登录（推荐扫码）。
6. 登录成功后，脚本会自动检测并将 Cookie 写入会话，无需手动复制。

#### 方式二：手动获取
1. 登录小红书网页版
2. 打开浏览器开发者工具（F12）
3. 切换到 "网络" 标签
4. 刷新页面，找到任意一个 API 请求
5. 复制请求头中的 `cookie` 字段
6. 在 Web 界面中粘贴到 Cookie 输入框，点击 "保存 Cookie"
...
- **关于导出密码**：
  - 导出所有笔记时需要密码验证
  - 密码每天自动生成，基于当天日期的 SHA-256 哈希值前6位
  - 密码会在服务器启动时输出到控制台
  - 密码也会在浏览器控制台中输出（仅用于开发调试）
...
| 25/12/14 | - **前端优化**：调整"一键自动获取"按钮逻辑，移除 toast 提示，直接对接后端 API 进行自动化获取；<br/>- **体验优化**：Cookie 获取帮助弹窗增加图文引导；<br/>- **部署修复**：修复 Alibaba Cloud Linux 3 系统下 `epel-release` 源冲突导致的一键部署脚本报错问题。 |
| 26/01/14 | - **安全增强**：<br/>  1. 新增 CSRF 保护<br/>  2. 增强输入验证<br/>  3. 使用 session 替代全局变量<br/>  4. 新增 API 速率限制<br/>- **代码质量优化**：<br/>  1. 统一日志管理<br/>  2. 完善错误处理<br/>  3. 优化代码结构<br/>- **用户体验提升**：<br/>  1. 增强加载状态显示<br/>  2. 优化错误提示<br/>  3. 改进 UI 交互<br/>  4. 新增图片预览功能<br/>- **文档优化**：<br/>  1. 完善 README.md<br/>  2. 新增 API 文档<br/>  3. 优化使用说明 |
...
#### 获取自己的笔记
```
GET /api/my_notes?comment_count=10
```

**参数:**
- `comment_count`: 可选，要获取的评论数量，默认10条
...
**参数:**
- `url`: 用户主页 URL
- `crawl_count`: 可选，要抓取的笔记数量
- `comment_count`: 可选，每条笔记要获取的评论数量，默认10条
